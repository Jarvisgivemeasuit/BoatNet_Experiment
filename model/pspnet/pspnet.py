...
class PSPNet(nn.Module):
    def __init__(self, inplanes, num_classes, backbone, use_threshold, use_gcn):
        ...
        self.out_conv1 = nn.Conv2d(num_classes, num_classes, 1, bias=False)
        self.out_conv2 = nn.Conv2d(num_classes, num_classes, 1, bias=False)
        self.use_threshold = use_threshold

    def forward(self, x):
        ori_x = x
        size = (x.shape[2], x.shape[3])
        x0, x1, x = self.backbone(x)
        ...
        return (output, out1, None, ratios) if self.use_threshold else out
        # return out1, out2
